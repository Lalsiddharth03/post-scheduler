...
export default function ScheduledPostsPage() {
  const { posts, loading, deletePost } = usePosts('SCHEDULED')
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400">Posts waiting to be published automatically</p>
        </div>
      ...
      {posts.length === 0 ? (
        <Card className="bg-slate-800/50 border-slate-700">
          <CardContent className="py-12 text-center">
            <Clock className="h-12 w-12 text-amber-500/50 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-white mb-2">No scheduled posts</h3>
            <p className="text-slate-400 mb-4">Schedule a post to publish it automatically</p>
            <Link href="/dashboard/create">
              <Button className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700">
                <Plus className="h-4 w-4 mr-2" />
                Create Post
